        ...
        if len(operation_schedule) == 1:
            ...
            for i in range(1, len(operation_schedule)):
                ...
                if op_earliest_start <= operation_schedule[i - 1].finish:
                    ...
                    if (
                        operation_schedule[i].start - operation_schedule[i - 1].finish
                        >= op_duration
                    ):
...
class Job(JobInfo):
    def __init__(self, job_info, index, deadline):
        super().__init__(
            job_info["name"], job_info["color"], job_info["operations"], index
        )
        self.index = index
...
class Operation:
    def __init__(self, operation_info, job_id, color, job_index=None):
        ...
        self.earliest_start = (
            operation_info["earliest_start"] if operation_info["earliest_start"] else 0
        )
...
class customRepeatableScheduler:
    def __init__(
        ...
        jobs,
        machines,
        cost_deadline_per_time,
        cost_hole_per_time,
        cost_processing_per_time,
        cost_makespan_per_time,
        profit_per_time,
        current_repeats,
        max_time=150,
        num_of_types=4,
    ) -> None:
        self.machines = [Machine(machine_info) for machine_info in machines]
        self.job_infos = [
            JobInfo(job_info["name"], job_info["color"], job_info["operations"])
        ...
        self.operations = [
            ...
            for job in job_list
            for operation in job.operation_queue
        ]
        ...
        self.machine_operation_rate = np.zeros(len(self.machines), dtype=np.float32)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.legal_actions = np.ones((len(self.machines), len(self.jobs)), dtype=bool)
        self.action_mask = np.ones((len(self.machines) * len(self.jobs)), dtype=bool)
    ...
    def _update_operation_state(self, action):
        ...
        if action is None:
            ...
            selected_operation = selected_job.operation_queue[
                self.schedule_buffer[action[1]][1]
            ]
            self.remain_op_duration_per_type[selected_operation.type].remove(
                selected_operation.duration // 100
            )
    ...
    def _update_machine_state(self, action=None):
        ...
        self.schedule_heatmap[action[0]] = np.array(
            self._schedule_to_array(operation_schedule)
        )
        ...
        self.machine_operation_rate = np.array(
            [machine.operation_rate for machine in self.machines]
        )
        ...
        for job_list in self.jobs:
            for job in job_list:
                remaining_operations = [
                    op for op in job.operation_queue if op.finish is None
                ]

                if not remaining_operations:
                    job.tardiness = job.operation_queue[-1].finish - job.deadline
                    job.time_exceeded = max(
                        0, job.operation_queue[-1].finish - job.deadline
                    )
                ...
                best_finish_times = [
                    machine.cal_best_finish_time(
                        op_earliest_start=earliest_operation.earliest_start,
                        op_type=earliest_operation.type,
                        op_duration=earliest_operation.duration,
                    )
                    for machine in self.machines
                ]
                ...
                scaled_rate = (
                    job.total_duration - sum(remaining_durations)
                ) / job.total_duration
    ...
    def test_cal_estimated_tardiness(self):
        for job in self.jobs[7]:
            ...
            print(
                f"Job 8 repeat {job.index} - Estimated Tardiness : {job.estimated_tardiness}"
            )
    ...
    def _schedule_to_array(self, operation_schedule):
        ...
        for operation in operation_schedule:
            ...
            for i in range(start, finish):
                ...
                encoded_job_repeat_schedule[i] = (
                    int(operation.job) + 1 + int(operation.job_index) * len(self.jobs)
                )
    ...
    def _schedule_operation(self, action):
        ...
        if selected_operation.predecessor is not None:
            predecessor_operation = next(
                ...
                if op.index == selected_operation.predecessor
            )
            operation_earliest_start = max(
                operation_earliest_start, predecessor_operation.finish
            )
    ...
    def get_observation(self):
        ...
        for job_list in self.jobs:
            ...
            cur_estimated_tardiness_per_job.append(
                job_list[0].estimated_tardiness / 100
            )
        ...
        mean_tardiness_per_job = [
            np.mean(job_tardiness) for job_tardiness in real_tardiness_per_job
        ]
        std_tardiness_per_job = [
            np.std(job_tardiness) for job_tardiness in real_tardiness_per_job
        ]
        ...
        for i, elem in enumerate(self.schedule_buffer):
            ...
            if elem[0] == -1:
                ...
                earliest_start_per_operation.append(
                    self.jobs[i][0].operation_queue[elem[1]].earliest_start // 100
                )
                job_deadline.append(self.jobs[i][0].deadline // 100)
                op_duration.append(
                    self.jobs[i][0].operation_queue[elem[1]].duration // 100
                )
                op_type.append(self.jobs[i][0].operation_queue[elem[1]].type)
                num_remaining_op.append(
                    len(
                        [
                            op
                            for op in self.jobs[i][0].operation_queue
                            if op.finish is None
                        ]
                    )
                )
                # remaining_working_time은 끝나지 않은 op들의 duration의 총합
                remaining_working_time.append(
                    sum(
                        [
                            op.duration // 100
                            for op in self.jobs[i][0].operation_queue
        ...
        # 머신 별 hole의 길이 계산
        hole_length_per_machine = [
            machine.cal_idle_time() // 100
            + min(machine.operation_schedule, key=lambda x: x.start).start // 100
        ...
        total_count_per_type = [
            len(self.remain_op_duration_per_type[i]) for i in range(self.num_of_types)
        ]
        # 아래 두 지표에서 empty list 예외 처리 필요
        mean_operation_duration_per_type = [
            np.mean(self.remain_op_duration_per_type[i])
            if self.remain_op_duration_per_type[i]
            else 0
            for i in range(self.num_of_types)
        ]
        std_operation_duration_per_type = [
            np.std(self.remain_op_duration_per_type[i])
            if self.remain_op_duration_per_type[i]
            else 0
            for i in range(self.num_of_types)
        ]
        ...
        observation_v4 = {
            ...
            "mean_operation_duration_per_type": np.array(
                mean_operation_duration_per_type
            ),
            "std_operation_duration_per_type": np.array(
                std_operation_duration_per_type
            ),
            # 현 scheduling 상황 관련 지표
            "last_finish_time_per_machine": np.array(
                [machine.cal_last_finish_time() // 100 for machine in self.machines]
            ),
            ...
            "schedule_buffer_operation_index": np.array(
                schedule_buffer_operation_index
            ),
            ...
            # 추정 tardiness 관련 지표
            "mean_estimated_tardiness_per_job": np.array(
                mean_estimated_tardiness_per_job
            ),
            "std_estimated_tardiness_per_job": np.array(
                std_estimated_tardiness_per_job
            ),
            "cur_estimated_tardiness_per_job": np.array(
                cur_estimated_tardiness_per_job
            ),
            # cost 관련 지표
        ...
        observation_v1 = {
            ...
            # cost 관련 지표
            "cost_factor_per_time": np.array(
                [
                    self.cost_deadline_per_time,
                    self.cost_hole_per_time,
                    self.cost_processing_per_time,
                    self.cost_makespan_per_time,
                ]
            ),
            "current_costs": np.array(
                [
    ...
    def get_info(self):
        return {
            ...
            "machine_operation_rate": [
                machine.operation_rate for machine in self.machines
            ],
            "schedule_buffer": self.schedule_buffer,
            "job_estimated_tardiness": [
                job.estimated_tardiness for job_list in self.jobs for job in job_list
            ],
            "current_schedule": self.current_schedule,
            "job_deadline": [
                job.deadline for job_list in self.jobs for job in job_list
            ],
            "job_time_exceeded": [
                job.time_exceeded for job_list in self.jobs for job in job_list
            ],
            "job_tardiness": [
                job.tardiness for job_list in self.jobs for job in job_list
            ],
    ...
    def render(self, mode="seaborn", num_steps=0):
        ...
        scheduled_df = list(
            filter(
                lambda operation: operation["sequence"] is not None, current_schedule
            )
        )
        ...
        if scheduled_df.empty:
            ...
            ax.set_yticklabels(
                [
                    f"Machine {i}\n ability:{self.machines[i].ability}"
            ...
            for i in range(len(self.machines)):
                ...
                for index, operation in machine_operations.iterrows():
                    ...
                    shaded_color = tuple(
                        [
                            min(max(shade * shade_factor, 0), 1) if idx < 3 else shade
                            for idx, shade in enumerate(base_color)
                        ]
                    )

                    # Build operation sequence string
                    operation_sequences = scheduled_df[
                        (scheduled_df["job"] == operation["job"])
                        & (scheduled_df["job_index"] == operation["job_index"])
                    ].sort_values(by="sequence")
                    operation_info = " -> ".join(
                        map(str, operation_sequences["machine"].tolist())
                    )
                    ...
                    op_block = mpatches.Rectangle(
                        (operation["start"], i - 0.5),
                        operation["finish"] - operation["start"],
                        1,
            ...
            ax.legend(
                handles=legend_patches, bbox_to_anchor=(1.01, 1), loc="upper left"
            )
    ...
    def is_done(self):
        ...
        return not np.any(self.action_mask) or all(
            [
                job.operation_queue[-1].finish is not None
                for job_list in self.jobs
                for job in job_list
            ]
        )
    ...
    def cal_entire_cost(self):
        self.cost_makespan = (
            self._get_final_operation_finish() * self.cost_makespan_per_time / 100
        )

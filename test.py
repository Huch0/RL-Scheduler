...
def custom_exp_schedule(progress_remaining):
    initial_learning_rate = 0.0003
    final_learning_rate = 0.00003
    middle_progress = 0.8

    if progress_remaining > middle_progress:
        # progress_remaining이 0.8에서 1 사이일 때, 지수적으로 감소
        a = (initial_learning_rate - final_learning_rate) * math.exp(1) / (math.exp(1) - 1)
        b = initial_learning_rate - a
        return a * math.exp(5*(progress_remaining-1)) + b
    else:
        # progress_remaining이 0.8 이하일 때, 고정된 학습률 반환
...
# net_arch=[1024, 256, 256, 64]
    ...
    params = {
        "policy_kwargs": dict(
            net_arch=dict(
                pi=[512, 256, 128, 128],  # 정책 네트워크 아키텍처
                vf=[512, 256, 256, 256]   # 가치 네트워크 아키텍처
            ),
        ), 
        "learning_rate" : custom_exp_schedule,
        "gamma": 1,
        "gae_lambda": 0.99,
        "n_steps": 4096,
        "batch_size": 256,
    ...
    env4, _  = make_env(num_machines = 8, num_jobs = 12, max_repeats = 12, repeat_means = [3] * 12, repeat_stds = [1] * 12, test_mode = True, cost_list = cost_list, profit_per_time = profit_per_time, max_time = max_time)

    # model = train_model(env = env4, env_name= "Single_Env4_gamma_1_obs_v4_clip_1_lr_custom_exp", eval_env= env4, params=params,version= "v1", total_steps= 5000000, deterministic = False)
    # test_model(env=env4, model=model, deterministic=True)


    # model = MaskablePPO.load("MP_Single_Env4_gamma_1_obs_v4_clip_1v1", env = env4, **params)#, env = env4, policy_kwargs=params["policy_kwargs"])
    # log_path = "./logs/tmp/Single_Env4_gamma_1_obs_v4_clip_1"
    ...
    # model.learn(total_timesteps=1000000, callback=maskable_eval_callback)
    # model.save("MP_Single_Env4_gamma_1_obs_v4_clip_1v2")
    ...
    model_path = 'MP_Single_Env4_gamma_1_obs_v4_clip_1_lr_custom_expv1_15000000'
    ...
    # repeat = repeats[0]
    # env, _ = make_env(num_machines = 8, num_jobs = 12, max_repeats = 12, repeat_means = repeat, repeat_stds = [1] * 12, test_mode = True, cost_list = cost_list, profit_per_time = profit_per_time, max_time = max_time)
    test_model(env=env4, model=model, deterministic=True, render=True, debug_step=[80, 81])
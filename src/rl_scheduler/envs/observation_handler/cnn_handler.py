...
class CNNHandler(ObservationHandler):
    def __init__(
        ...
        # for CNN
        self.image_height = max(36, self.M)
        ...
        return spaces.Dict(
            {
                ...
                "schedule_heatmap": spaces.Box(
                    low=0,
                    high=255,
                    shape=(3, self.image_height, self.time_horizon),
                    dtype=np.uint8,
                ),
    ...
    def get_observation(self) -> Dict[str, Any]:
        ...
        schedule_heatmap = np.zeros(
            (3, self.image_height, self.time_horizon), dtype=np.uint8
        )
        # (type, duration, index)
        for machine_id, machine in enumerate(scheduler.machine_instances):
            for op in machine.assigned_operations:
                ...
                schedule_heatmap[0, machine_id, start_time:end_time] = self.type_index[
                    op.type_code
                ]
                schedule_heatmap[1, machine_id, start_time:end_time] = op.duration
                schedule_heatmap[2, machine_id, start_time:end_time] = (
                    op.operation_template.operation_template_id
                )

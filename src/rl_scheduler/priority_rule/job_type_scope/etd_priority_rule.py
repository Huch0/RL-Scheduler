    ...
    def assign_priority(self) -> List[int]:
        ...
        def estimate_finish(machine, op) -> int:
            slots = sorted(
                [
                    ...
                    for o in machine.assigned_operations
        ...
        for job_group in self.scheduler.job_instances:
            ...
            for job in unfinished:
                remaining = [
                    op for op in job.operation_instance_sequence if op.end_time is None
                ]
                if not remaining:
                    ...
                    finish_times = [
                        ...
                        if op0.type_code
                        in machine.machine_template.supported_operation_type_codes
                    ]
                    if finish_times:
                    ...
                    total_dur = sum(
                        op.duration for op in job.operation_instance_sequence
                    )
                    rem_durs = [op.duration for op in remaining[1:]]
                    progress_scale = (
                        (total_dur - sum(rem_durs)) / total_dur if total_dur > 0 else 0
                    )

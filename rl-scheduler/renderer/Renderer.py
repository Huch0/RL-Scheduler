...
class Renderer:
    ...
    def render_gantt(machine_instances, render_info_path: Path, title="Gantt Chart"):
        ...
        for m_idx, machine in enumerate(machine_instances):
            for op in machine.assigned_operations:
                if op.start_time is not None and op.end_time is not None:
                    # job_template_id, job_instance_id를 함께 저장
        ...
        for _, row in df.iterrows():
            ...
            # 레전드 라벨 예) Job0-1
            job_label = f"Job{job_template_id}-{job_instance_id}"

            # job_template_id 기반으로 색상 선택
            color_map = render_info.get("job_colors", {})
            job_color = color_map.get(str(job_template_id), "#cccccc")
            ...
            # 레전드용 정보: (job_template_id, job_instance_id, 라벨, 색)
            if job_label not in [x[2] for x in legend_info]:
                legend_info.append((job_template_id, job_instance_id, job_label, job_color))

        # (job_template_id, job_instance_id) 순으로 정렬

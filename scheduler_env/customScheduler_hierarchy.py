...
class CustomSchedulerHierarchy():
    def __init__(self, jobs, machines, cost_deadline_per_time, cost_hole_per_time, cost_processing_per_time, cost_makespan_per_time, profit_per_time, current_repeats, max_time = 150, num_of_types = 4) -> None:
        ...
        self.action_mask = np.ones(
            shape=max(len(self.machines), len(self.jobs)), dtype=bool)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.action_mask = np.ones(
            max(len(self.machines), len(self.jobs)), dtype=bool)
    ...
    def update_action_masks(self, action, current_phase = None):
        if action is None:
            return

        if current_phase == "machine_selection":
            for i, job_list in enumerate(self.jobs):
                if all(job.is_done for job in job_list):
                    self.action_mask[i] = False
        
        if current_phase is None:
            selected_operation = self.jobs[action[1]][0].operation_queue[self.schedule_buffer[action[1]][1]]
            for i, machine in enumerate(self.machines):
                if not machine.can_process_operation(selected_operation.type):
                    self.action_mask[i] = False
    ...
    def update_state(self, action=None):
        if action is not None:
            ...
            self.update_action_masks(action, "machine_selection")
            ...
            self._update_machine_state(action)
            self._update_job_state()
            self.update_action_masks(action)
    ...
    def is_legal(self, action):
        job = action[1]
        machine = action[0]
        if self.action_mask[machine][job]:
            return True
        else:
            return False
